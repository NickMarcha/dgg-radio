...
- `chart-template.jsx` - Starter template for a basic chart
- `interactive-template.jsx` - Template with tooltips, zoom, and interactions
- `sample-data.json` - Example datasets for testing

Both templates are React components built on hooks and a ref, which is Pattern A above. The d3 inside them transfers to vanilla JavaScript, Vue or Svelte unchanged; the component around it does not.

To use these resources, read the relevant files when detailed guidance is needed for specific visualisation types or patterns.

## In this repository

**d3 7.9 is a dependency**, with `@types/d3`, and the charts are TypeScript. The
one chart the room draws is `src/components/EmbedHistoryChart.tsx`: every
channel destiny.gg was watching on one axis, with a brushed strip under it.

Read that file before writing another chart here, because it settles the two
questions this repository has already answered.

**d3 does the arithmetic and React does the DOM.** Scales, `line().defined()`,
tick choice, time formatting and `bisectCenter` are d3's; every element is JSX.
That is not a stylistic preference: the admin page is an Astro island, so a
chart that built its own nodes would render nothing on the server and the whole
thing can be asserted against as markup instead of through a fake DOM. The one
exception is `brushX`, which owns the handful of nodes it drags — it lives in an
effect against a ref, React owns nothing under it, and only `end` is listened to
because each selection costs a request. Reach for `d3.select` only where the
thing being built genuinely moves between renders.

**The palette is already decided.** Eight categorical hues, in a fixed order,
checked against this room's `#1f2023` surface for lightness, chroma, contrast
and colour-vision separation. Use them; `d3.schemeCategory10` and
`interpolateYlOrRd` are picked for white and will not survive here. A channel's
colour comes from its own name rather than its rank, so a filter never repaints
the lines that survive it — but a name alone cannot finish the job, because
eight hues and any number of channels means collisions. Resolving one by taking
the next free hue makes the answer depend on who else is drawn, so the
assignment is held across renders and a hue is handed on only once nothing is
using it. Any palette of fixed size has this problem; solve it before shipping a
chart somebody can toggle. The built-in `dataviz` skill carries the validator
that settled those eight, and is worth loading before adding a ninth.

Two smaller things that have already cost time here:

- Astro prerenders an island's first paint on the server, where there is no
  layout. Anything measured from the DOM — `getBoundingClientRect`, a
  `ResizeObserver`, `window.innerWidth`, and `d3.pointer`'s SVG path — has to
  run in an effect or a handler. A hydration mismatch does not throw; React
  keeps the server's markup and the page quietly stays wrong.
- A hidden browser tab runs no animation at all, so `transition()` cannot be
  checked through a screenshot. Drive it by hand or test the numbers.

## Source

Vendored from `chrisvoncsefalvay/claude-d3js-skill` at `e198c87`, which carries
no licence file. The asset filenames and the claim that the templates are
framework-agnostic were corrected against what that repository actually
contains, and the section above was added.
